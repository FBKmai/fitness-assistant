...
**容错设计**：AI 教练 / 饮食教练 / 餐食建议**不再做本地规则兜底**——AI 调用失败时直接把错误提示给用户，并通过 `AppLog.error(...)` 记到「更多 → 调试日志」（`AppLogStore` + `DebugLogView`），不要再加伪装成 AI 回复的本地文案。仍保留的合理降级：`CalorieCalculator` 在缺少 HealthKit 静息能量时回退到基于 `UserProfile` 的 Mifflin-St Jeor BMR 公式；`TodayView` 的「今日」摘要用本地 `FatLossAnalyzer` 文案（这是仪表盘自身内容，不是 AI 兜底）。新增 `catch` 时统一调用 `AppLog.error(_:category:)` 把报错汇总到调试日志。
